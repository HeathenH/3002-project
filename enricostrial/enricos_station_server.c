...
void setup_shared_memory() {
    shmid = shmget(IPC_PRIVATE, sizeof(ServerState), IPC_CREAT | 0666);
...
void process_journey_request(const char *destination, const char *current_time) {
    printf("Processing journey to %s after %s\n", destination, current_time);
    int found = 0;
    for (int i = 0; i < TIMETABLE_SIZE; i++) {
        // Check if the destination matches and the departure time is after the current time
        if (strcmp(serverState->timetable[i][4], destination) == 0 && strcmp(serverState->timetable[i][0], current_time) >= 0) {
    ...
    if (!found) {
        printf("No journey found to %s after %s\n", destination, current_time);
    }
}
...
void tcp_server_loop() {
    ...
    while (1) {
        ...
        if (n > 0) {
            ...
            char destination[50], current_time[6];
            sscanf(path, "/?to=%49[^&]&Time=%5s", destination, current_time);

            process_journey_request(destination, current_time);

            // Respond with a simple HTTP response
            char response[] = "HTTP/1.1 200 OK\r\nContent-Type: text/plain\r\n\r\nJourney details have been processed.";
...
void udp_server_loop() {
    ...
    while (1) {
        ...
        if (n > 0) {
            ...
            // Process the UDP message (assuming a simple format)
            char destination[50], current_time[6];
            sscanf(buffer, "%49[^&]&%5s", destination, current_time);
