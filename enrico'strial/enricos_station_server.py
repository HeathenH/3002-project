...
class NetworkServer(multiprocessing.Process):
    def __init__(self, station_name, browser_port, query_port, adjacent_ports):
        ...
        self.adjacent_ports = [(port_str.split(':')[0], int(port_str.split(':')[1])) for port_str in adjacent_ports]
        self.host_ip = "0.0.0.0"  # Bind to all interfaces
    ...
    def run(self):
        ...
        while len(self.station_list) != len(self.adjacent_ports):
            ...
            for ip, port in self.adjacent_ports:
                try:
                    neighboring_station_address = (ip, port)
                    self.udp_socket.sendto(query_data.encode("utf-8"), neighboring_station_address)
                    print(f"Sent query to {neighboring_station_address}")
                    time.sleep(1)
                except socket.error as e:
                    print(f"Error sending query to {neighboring_station_address}: {e}")
    ...
    def handle_tcp(self):
        try:
            while True:
                tcp_conn, addr = self.tcp_socket.accept()
                ...
                if request_data:
                    ...
                    if http_method == "GET" and http_path.startswith("/?to="):
                        ...
                        if journey_list and journey_list[0][0][-1] == "midnight":
                            journey_list = []
                ...
                try:
                    ...
                    if "query_station" in data.decode("utf-8"):
                        ...
                        try:
                            ...
                            print(f"Error sending station data to {addr}: {e}")
                        ...
                        if "ended" in data.decode("utf-8") or "midnight" in data.decode("utf-8"):
                            ...
                            if len(journey_list[2]) > 1:
                                ...
                                try:
                                    ...
                                    print(f"Sent journey data to {address}")
                                except socket.error as e:
                                    print(f"Error sending journey data to {address}: {e}")
                            else:
                                print("end pinged")
                                self.journey_list = []
                                    ...
                                    if self.counter > 0:
                                        ...
                                        if self.temp_list[-1][-1] == self.temp_list[0][2]:  # Compare latest journey, end journey if match
                                            ...
                                            try:
                                                ...
                                                print(f"Error sending final journey data to {address}: {e}")
                                        else:
                                            for port in self.station_list:
                                                station_name = port[0]
                                                port_number = port[1]
                                                ...
                                                if station_name == self.temp_list[-1][-1]:
                                                    ...
                                                    journey_data = self.temp_list
                                                    adjacent_addr = (station_name, port_number)
                                                    try:
                                                        self.udp_socket.sendto(json.dumps(journey_data).encode("utf-8"), adjacent_addr)
                                                        print(f"Sent journey data to {adjacent_addr}")
                                                    except socket.error as e:
                            ...
                            if midnight == 0:
                                ...
                                try:
                                    self.udp_socket.sendto(json.dumps(journey_data).encode("utf-8"), address)
                                    print(f"Sent midnight journey data to {address}")
                                except socket.error as e:
                                    print(f"Error sending midnight journey data to {address}: {e}")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) <= 4:
        print("Usage: python3 ./station-server station-name browser-port query-port neighbour1 [neighbour2 ...]")
        sys.exit(1)

    station_name = sys.argv[1]
    browser_port = sys.argv[2]
    query_port = sys.argv[3]
    adjacent_ports = sys.argv[4:]
