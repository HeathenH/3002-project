            ...
            while True:
                ...
                if request_data:
                    ...
                    if http_method == "GET" and http_path.startswith("/?to="):
                        #print("Processing valid TCP request")
                        # Handle TCP request
                        ...
                        # Send UDP query to each adjacent port
                        ...
                        for port in self.adjacent_ports:
                            ...
                            print(f"sent query_timetable to {port}")
                        #print("Sent TCP response")
                    else:
        ...
        try:
            while True:
                ...
                if "query_timetable" in data.decode("utf-8"):
                    ...
                    if timetable_data:
                        self.udp_socket.sendto(timetable_json.encode("utf-8"), addr)
                        print(f"sent timetable to {addr}")
                    else:
                        print("Timetable data is not available.")
                        
                if "station-name" in data.decode("utf-8"):
                    #print("Processing station-name UDP request")
                    received_data = data.decode("utf-8")
                    #print("Received data:", received_data)
                    received_timetable_data = json.loads(received_data)
                    #received_timetable_data = json.loads(data.decode("utf-8"))
                    print(f"Timetable received: {received_timetable_data}")
